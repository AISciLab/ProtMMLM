...
TARGETS = ("vdw", "hbbb", "hbsb", "hbss", "hp", "sb", "pc", "ps", "ts")
...
FRAME_TARGET_FIELDS = ["protein_id", "frame_id", "split", *TARGETS]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Frozen Fusion global embedding probes for frame-level 9-term energy prediction."
    )
    ...
    parser.add_argument("--energy-csv", type=Path, default=Path("data/downstream/all_interactions_all.csv"), help="Frame-level energy CSV with sample, frame, and 9 energy-term columns.")
    parser.add_argument(
        "--energy-frame-offset",
        type=int,
        default=1,
        help="Offset added to zero-based MD frame indices before matching the energy CSV frame column. Default 1 matches all_interactions_all.csv's 1-based frames.",
    )
    parser.add_argument("--split-file", type=Path, default=None, help="Existing protein-level split CSV. If omitted, a new MMseqs split is built before probing.")
    ...
    parser.add_argument("--target-min", type=float, default=None, help="Accepted for CLI compatibility; not applied for multi-target 9-term energy probing.")
    parser.add_argument("--target-max", type=float, default=None, help="Accepted for CLI compatibility; not applied for multi-target 9-term energy probing.")
    parser.add_argument("--target-quantile-low", type=float, default=None, help="Accepted for CLI compatibility; not applied for multi-target 9-term energy probing.")
    parser.add_argument("--target-quantile-high", type=float, default=None, help="Accepted for CLI compatibility; not applied for multi-target 9-term energy probing.")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional .npz cache for labeled frame-level frozen Fusion embeddings and 9 energy-term targets.",
    )
    parser.add_argument(
        "--reuse-embeddings",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    split_file = resolve_or_build_split_file(args, output_dir)
    ...
    if args.reuse_embeddings:
        ...
        dataset = PretrainDataset.from_manifest(config.manifest_path, sample_limit=None, full_only=True)
        ...
        if not samples:
            raise ValueError("No samples matched manifest/split filters.")
    ...
    if filter_summary["num_filtered"] > 0:
        ...
        print(
            "[filter] "
            f"target=energy_terms before={filter_summary['num_before']} after={filter_summary['num_after']} "
    ...
    resolved = {
        ...
        "probes": probes,
        "targets": TARGETS,
        "note": "Frozen representation probing only; ProtMMLM weights are not updated; only probes are trained. Targets are the 9 non-total energy terms.",
    }
    _write_json(output_dir / "resolved_config.json", resolved)
...
def resolve_or_build_split_file(args: argparse.Namespace, output_dir: Path) -> Path:
    if args.split_file is not None:
        return _resolve_repo_path_required(args.split_file)

    cluster_seed = int(args.cluster_seed if args.cluster_seed is not None else args.seed)
    split_output_dir = _resolve_output_path(args.split_output_dir) if args.split_output_dir is not None else output_dir / "splits"
    split_output_dir.mkdir(parents=True, exist_ok=True)
    identity_label = _format_identity_label(float(args.cluster_min_seq_id))
    split_file = split_output_dir / f"protein_mmseqs{identity_label}_seed{cluster_seed}.csv"
    summary_file = split_output_dir / f"protein_mmseqs{identity_label}_seed{cluster_seed}_summary.json"
    summary = split_md_pairwise_manifest_by_mmseqs(
        manifest_path=_resolve_repo_path_required(args.split_source_manifest),
        output_manifest_path=split_file,
    ...
    with torch.inference_mode():
        for index, sample in enumerate(samples):
            try:
                ...
                matched_scores: list[dict[str, float]] = []
                        ...
                        failures.append(
                            {
                                ...
                                "error_message": "No 9-term energy vector for (sample, frame).",
                            }
                        )
                        continue
                    energy_values = dict(energy_targets[key])
                    frame_rows.append(
                        {
                            "protein_id": sample.protein_id,
                            "frame_id": energy_frame_id,
                ...
                if args.save_embeddings and matched_embeddings:
                    np.savez_compressed(
                        ...
                        **{target: np.asarray([row[target] for row in matched_scores], dtype=np.float32) for target in TARGETS},
                    )
...
def load_energy_targets(path: Path) -> dict[tuple[str, int], dict[str, float]]:
    if not path.exists():
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        required_columns = {"sample", "frame", *TARGETS}
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            for target in TARGETS:
                raw_value = str(row.get(target, "")).strip()
                try:
                    value = float(raw_value)
                except ValueError as exc:
                    raise ValueError(f"Energy CSV has invalid {target} value {raw_value!r} at row {row_number}: {path}") from exc
                if not math.isfinite(value):
                    raise ValueError(f"Energy CSV has non-finite {target} value at row {row_number}: {path}")
                energy_values[target] = value
            key = (sample, frame_id)
            if key in targets:
                raise ValueError(f"Energy CSV contains duplicate (sample, frame) key {key}: {path}")
            targets[key] = energy_values
    if not targets:
...
def save_embedding_cache(path: Path, embedding_rows: list[dict[str, Any]]) -> None:
    ...
    target_arrays = {target: np.asarray([float(row["targets"][target]) for row in embedding_rows], dtype=np.float32) for target in TARGETS}
    np.savez_compressed(
        path,
...
def load_embedding_cache(
    ...
    *,
    ...
    required = {"protein_ids", "frame_ids", "embeddings", *TARGETS}
    ...
    target_arrays = {target: np.asarray(payload[target], dtype=np.float32) for target in TARGETS}
    frame_count = int(protein_ids.shape[0])
    if not (
        frame_ids.shape[0] == frame_count
        and embeddings.shape[0] == frame_count
        and all(target_arrays[target].shape[0] == frame_count for target in TARGETS)
    ):
    ...
    for index in range(frame_count):
        ...
        targets = {target: float(target_arrays[target][index]) for target in TARGETS}
        frame_rows.append(
            {
...
def filter_embedding_rows_by_target(embedding_rows: list[dict[str, Any]], *, args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "target_min": None,
        "target_max": None,
        "target_quantile_low": None,
        "target_quantile_high": None,
        "lower_bound": None,
        "upper_bound": None,
        "num_before": len(embedding_rows),
        "num_after": len(embedding_rows),
        "num_filtered": 0,
    }
    ...
    return [
        {
            ...
            "split": row["split"],
            **{target: float(row["targets"][target]) for target in TARGETS},
        }
