...
FRAME_TARGET_FIELDS = ["protein_id", "frame_id", "split", "rg", "re", "asphericity"]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Frozen Fusion global embedding probes for frame-level Rg/Re/asphericity prediction."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional .npz cache for all frame-level frozen Fusion embeddings and targets.",
    )
    parser.add_argument(
        "--reuse-embeddings",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.reuse_embeddings:
        ...
        if failures:
            write_csv(output_dir / "failures.csv", failures, ["protein_id", "stage", "error_type", "error_message"])
    ...
    if not frame_rows:
        raise ValueError("No frame rows were extracted.")
    ...
    resolved = {
        ...
        "probes": probes,
        "targets": TARGETS,
        "note": "Frozen representation probing only; ProtMMLM weights are not updated.",
    }
    _write_json(output_dir / "resolved_config.json", resolved)
    ...
    *,
    ...
    with torch.inference_mode():
        for index, sample in enumerate(samples):
            try:
                frames = load_md_ca_frames(sample.md_path, max_residues=config.max_residues, max_frames=config.max_frames)
                if len(frames) < 2:
                ...
                props = compute_frame_structural_properties(frames)
                for row_index, frame in enumerate(frames):
                    frame_id = int(getattr(frame, "frame_index", row_index))
                    frame_rows.append(
                        {
                            "protein_id": sample.protein_id,
                            "frame_id": frame_id,
                            "split": split,
                            "rg": float(props["rg"][row_index]),
                            "re": float(props["re"][row_index]),
                            "asphericity": float(props["asphericity"][row_index]),
                        }
                    )
                    embedding_rows.append(
                        {
                            ...
                            "targets": {target: float(props[target][row_index]) for target in TARGETS},
                        }
                    )
                if args.save_embeddings:
                    np.savez_compressed(
            ...
            if args.log_interval > 0 and (index + 1) % args.log_interval == 0:
                print(f"processed={index + 1}/{len(samples)} frames={len(frame_rows)} failures={len(failures)}", flush=True)
...
def save_embedding_cache(path: Path, embedding_rows: list[dict[str, Any]]) -> None:
    ...
    rg = np.asarray([float(row["targets"]["rg"]) for row in embedding_rows], dtype=np.float32)
    re = np.asarray([float(row["targets"]["re"]) for row in embedding_rows], dtype=np.float32)
    asphericity = np.asarray([float(row["targets"]["asphericity"]) for row in embedding_rows], dtype=np.float32)
    np.savez_compressed(
        path,
...
def load_embedding_cache(
    ...
    *,
    ...
    required = {"protein_ids", "frame_ids", "embeddings", "rg", "re", "asphericity"}
    ...
    rg = np.asarray(payload["rg"], dtype=np.float32)
    re = np.asarray(payload["re"], dtype=np.float32)
    asphericity = np.asarray(payload["asphericity"], dtype=np.float32)
    frame_count = int(protein_ids.shape[0])
    if not (
        frame_ids.shape[0] == frame_count
        and embeddings.shape[0] == frame_count
        and rg.shape[0] == frame_count
        and re.shape[0] == frame_count
        and asphericity.shape[0] == frame_count
    ):
    ...
    for index in range(frame_count):
        ...
        targets = {
            "rg": float(rg[index]),
            "re": float(re[index]),
            "asphericity": float(asphericity[index]),
        }
        frame_rows.append(
            {
...
def compute_frame_structural_properties(frames: list[Any]) -> dict[str, np.ndarray]:
    ...
    for frame in frames:
        ...
        if coords.ndim != 2 or coords.shape[1] != 3 or coords.shape[0] < 2:
            raise ValueError(f"Invalid C-alpha coordinate shape: {coords.shape}")
        center = coords.mean(axis=0)
        centered = coords - center
        rg_values.append(float(np.sqrt(np.mean(np.sum(centered * centered, axis=1)))))
        re_values.append(float(np.linalg.norm(coords[-1] - coords[0])))
        tensor = (centered.T @ centered) / float(coords.shape[0])
        eigvals = np.sort(np.linalg.eigvalsh(tensor))[::-1]
        l1, l2, l3 = [float(value) for value in eigvals]
        denom = (l1 + l2 + l3) ** 2
        if denom <= 1.0e-24:
            asp_values.append(0.0)
        else:
            asp_values.append(((l1 - l2) ** 2 + (l2 - l3) ** 2 + (l3 - l1) ** 2) / (2.0 * denom))
    return {
        "rg": np.asarray(rg_values, dtype=np.float32),
        "re": np.asarray(re_values, dtype=np.float32),
        "asphericity": np.asarray(asp_values, dtype=np.float32),
    }
